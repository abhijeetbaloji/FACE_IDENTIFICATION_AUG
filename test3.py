...
# # Define a minimum visible landmark count threshold (50% of 68 landmarks for example)
# MIN_VISIBLE_LANDMARKS = 34

# def is_partial_face_visible(landmarks):
#     """Check if at least 50% of the face landmarks are detected."""
#     return len(landmarks) >= MIN_VISIBLE_LANDMARKS
...
#         # Check if only part of the face is visible
#         if landmarks and is_partial_face_visible(landmarks):
#             # Use only visible landmarks for verification
...
#             print(len(saved_avg_landmarks))
#             print(len(new_landmarks_list))
#             return None
...
def open_webcam_with_orientation_landmarks():
    ...
    while True:
        ...
        if landmarks and is_partial_face_visible(landmarks):
            # Use only visible landmarks for verification
            frames_landmarks[orientation].append(np.array(landmarks))
            print(f"Landmarks stored for '{orientation}' orientation. Total frames stored: {len(frames_landmarks[orientation])}")
        else:
            print(f"Insufficient landmarks for '{orientation}' orientation; not storing this frame.")
...
def calculate_similarity_score(new_landmarks_list, orientation, json_file="averaged_landmarks_by_orientation.json"):
    try:
        ...
        # Check if saved landmarks or new landmarks are missing
        if not saved_avg_landmarks.any():
            print(f"No saved landmarks data found for '{orientation}' orientation.")
            return None
        if not new_landmarks_list:
            print(f"No new landmarks data found for '{orientation}' orientation.")
            return None

        # Calculate mean of new landmarks if available
        new_avg_landmarks = np.mean(np.array(new_landmarks_list), axis=0)
